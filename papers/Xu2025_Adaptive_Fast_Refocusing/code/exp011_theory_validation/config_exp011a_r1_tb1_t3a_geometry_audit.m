function cfg = config_exp011a_r1_tb1_t3a_geometry_audit(run_mode)
%CONFIG_EXP011A_R1_TB1_T3A_GEOMETRY_AUDIT
...
%% Identity
cfg.experiment_id = "EXP011A_R1_TB1_T3A";
cfg.experiment_name = "TB1_Retrospective_Objective_Geometry_Audit_R1";
...
% R1 audit fix: objective-geometry coverage is NOT the same as the frozen
% operational GlobalReference domain. Geometry must cover every local
% window reachable from the frozen seed bank so that a G0/N3 branch is
% never marked unresolved merely because its peak lies outside [-1.5,1.5].
% These limits are derived from frozen search geometry, not from labels.
cfg.geometry_search_lo_bins = cfg.seed_min - cfg.local_search_halfwidth_bins;
cfg.geometry_search_hi_bins = cfg.seed_max + cfg.local_search_halfwidth_bins;
...
cfg.output_dir = fullfile( ...
    cfg.output_root,"exp011a_r1_tb1_t3a_geometry_audit",char(run_mode));
cfg.figure_dir = fullfile(cfg.output_dir,"figures");

cfg.feedback_bundle_name = "EXP011A_R1_TB1_T3A_FEEDBACK_BUNDLE.txt";
